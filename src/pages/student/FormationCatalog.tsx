...
const FormationCatalog: React.FC = () => {
  ...
  const publishedFormations = allFormations.filter((f) => f.status === 'published');

  // Apply search and filters
  const filteredFormations = useMemo(() => {
    return publishedFormations.filter((formation) => {
      const matchesSearch =
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={levelFilter}
                onChange={(e) => setLevelFilter(e.target.value as FormationLevel | 'all')}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              {levelFilter !== 'all' && (
                <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs">
                  Niveau: {getLevelBadge(levelFilter).props.children}
                </span>
              )}
              <button
                onClick={() => {
